...
def parsear_estadisticas_compactas(stats_data: dict) -> dict:
    ...
    for p in periodos:
        ...
        if p_name in ("total", "all", "overall"):
            periodo_total = p
            break
...
def descargar_stats_partido(event_id: int) -> dict:
    ...
    xg = parsear_xg_compacto(
        api_get(f"https://api.sofascore.com/api/v1/event/{event_id}/shotmap")
    )
    stats.update(xg)
    return stats
...
def procesar_dia(fecha: str) -> int:
    ...
    eventos = [
        ...
        if e.get("tournament", {}).get("uniqueTournament", {}).get("id") in IDS_DESEADOS
        and es_partido_finalizado(e)
    ]
    ...
    for i, evento in enumerate(eventos, 1):
        ...
        try:
            ...
            h_sc = evento.get("homeScore", {}) or {}
            a_sc = evento.get("awayScore", {}) or {}
