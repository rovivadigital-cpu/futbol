...
TORNEOS_IDS = {
    17:    {"nombre": "Premier League",                 "pais": "England",                 "equipos_muestra": ["arsenal", "chelsea", "tottenham", "aston villa", "newcastle", "brentford", "brighton", "crystal palace", "everton", "fulham"]},
    ...
    19:    {"nombre": "FA Cup",                         "pais": "England",                 "equipos_muestra": ["arsenal", "chelsea", "tottenham", "aston villa", "newcastle"]},
    841:   {"nombre": "Algerian Ligue 1",               "pais": "Algeria",                 "equipos_muestra": ["mouloudia", "usm alger", "es setif", "cr belouizdad", "js kabylie", "mc oran"]},
    155:   {"nombre": "Liga Profesional de Fútbol",     "pais": "Argentina",               "equipos_muestra": ["boca juniors", "talleres de cordoba", "velez sarsfield", "san lorenzo", "argentinos juniors", "barracas central", "instituto", "rosario central"]},
    ...
    11539: {"nombre": "Primera A, Apertura",            "pais": "Colombia",                "equipos_muestra": ["millonarios", "santa fe", "bucaramanga", "once caldas", "deportes tolima", "boyaca chico", "fortaleza ceif", "jaguares de cordoba"]},
    11536: {"nombre": "Primera A, Finalización",        "pais": "Colombia",                "equipos_muestra": ["millonarios", "santa fe", "bucaramanga", "once caldas", "deportes tolima", "boyaca chico", "fortaleza ceif", "jaguares de cordoba"]},
    ...
    240:   {"nombre": "LigaPro Serie A",                "pais": "Ecuador",                 "equipos_muestra": ["barcelona sc", "emelec", "liga de quito", "aucas", "independiente del valle", "delfin", "orense", "macara"]},
    ...
    11454: {"nombre": "Campeones Cup",                  "pais": "North & Central America", "equipos_muestra": ["la galaxy", "seattle sounders", "tigres uanl", "monterrey"]},
    498:   {"nombre": "CONCACAF Champions Cup",         "pais": "North & Central America", "equipos_muestra": ["tigres uanl", "monterrey", "la galaxy", "toronto", "seattle sounders", "pachuca"]},
    13783: {"nombre": "Leagues Cup",                    "pais": "North & Central America", "equipos_muestra": ["la galaxy", "inter miami", "tigres uanl", "seattle sounders", "cruz azul"]},
    384:   {"nombre": "CONMEBOL Libertadores",          "pais": "South America",           "equipos_muestra": ["flamengo", "boca juniors", "palmeiras", "atletico mineiro", "peñarol", "cerro porteño", "millonarios"]},
    480:   {"nombre": "CONMEBOL Sudamericana",          "pais": "South America",           "equipos_muestra": ["ldu quito", "independiente del valle", "fluminense", "defensa y justicia", "cruzeiro", "racing club"]},
    ...
    10602: {"nombre": "Copa Libertadores Femenina",     "pais": "South America",           "equipos_muestra": ["corinthians", "boca juniors", "ferroviaria", "america de cali", "colo colo"]},
    ...
    11540: {"nombre": "Primera División, Apertura",     "pais": "Paraguay",                "equipos_muestra": ["cerro porteño", "sportivo luqueño", "guarani", "tacuary", "sportivo trinidense", "sportivo ameliano", "general caballero"]},
    11541: {"nombre": "Primera División, Clausura",     "pais": "Paraguay",                "equipos_muestra": ["cerro porteño", "sportivo luqueño", "guarani", "tacuary", "sportivo trinidense", "sportivo ameliano", "general caballero"]},
    ...
    278:   {"nombre": "Liga AUF Uruguaya",              "pais": "Uruguay",                 "equipos_muestra": ["peñarol", "defensor sporting", "montevideo wanderers", "boston river", "cerro largo", "danubio", "rampla juniors", "miramar misiones"]},
    ...
    11621: {"nombre": "Liga MX, Apertura",              "pais": "Mexico",                  "equipos_muestra": ["chivas", "tigres", "monterrey", "cruz azul", "pumas", "toluca", "pachuca", "tijuana"]},
    11620: {"nombre": "Liga MX, Clausura",              "pais": "Mexico",                  "equipos_muestra": ["chivas", "tigres", "monterrey", "cruz azul", "pumas", "toluca", "pachuca", "tijuana"]},
...
# Ajustado para GitHub Actions (crons menos frecuentes, ej. 3 al día)
# =====================================================================
MAX_LLAMADAS_POR_CORRIDA = 15  
...
def limite_alcanzado():
    ...
    if llamadas_realizadas >= MAX_LLAMADAS_POR_CORRIDA:
        print(f"\n   ⏸️  Límite de {MAX_LLAMADAS_POR_CORRIDA} llamadas por corrida alcanzado. "
              f"Pausando aquí para repartir la cuota diaria.", flush=True)
        return True
    return False
...
def llamar_gemini_json(prompt):
    """
    Llama a Gemini con la herramienta de búsqueda de Google.
    ...
    if not texto_limpio:
        ...
        # Si la respuesta es vacía por STOP, es que no encontró partidos (normal)
        if str(finish_reason) == "STOP" or str(finish_reason) == "FinishReason.STOP":
            return {}
        else:
...
# =====================================================================
# ROTACIÓN DE LIGAS CON GUARDADO SEGURO DE CURSOR
# =====================================================================
...
print(f"\n-> Iniciando revisión desde la liga en la posición [{cursor_ligas}] de {n_ligas}...", flush=True)

cuota_agotada_b = False
ligas_revisadas = 0  # Contador de ligas que realmente tocamos en este turno
...
for tor_id, info in LIGAS_ACTUALIZAR.items():
    ...
    ligas_revisadas += 1  # Empezamos a procesar esta liga
...
REGLAS ESTRICTAS:
- NO incluyas partidos de otras ligas aunque tengan un nombre similar.
- NO incluyas partidos de divisiones inferiores, copas ni torneos amistosos a menos que la liga solicitada sea una copa.
            ...
            if partidos:
                ...
                with open(csv_filename, "a", newline="", encoding="utf-8") as f:
                    ...
                    for p in partidos:
                        ...
                        # Validación de liga incorrecta
                        es_invalido, razon = equipo_parece_de_otra_liga(local, visita, tor_id)
                        if es_invalido:
                        ...
                        row = [
                            id_evento_calculado,               # event_id
                            pais_nombre,                       # pais
                            liga_nombre,                       # liga
                            tor_id,                            # tourney_id
                            liga_nombre,                       # tourney_name
                            season_detectada,                  # tourney_season
                        ...
                        writer.writerow(row)
                if partidos_nuevos_guardados > 0:
...
# =====================================================================
# GUARDAR EL CURSOR EXACTO AL FINAL
# =====================================================================
if ligas_revisadas > 0:
    nuevo_cursor = (cursor_ligas + ligas_revisadas) % n_ligas
    try:
        with open(estado_ligas_path, "w", encoding="utf-8") as f:
            json.dump({"indice_actual": nuevo_cursor}, f)
        print(f"\n-> Se revisaron {ligas_revisadas} ligas en este turno. Próximo turno iniciará en la posición: {nuevo_cursor}", flush=True)
    except Exception as e:
